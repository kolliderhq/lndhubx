...
type Identifier = Rc<Box<dyn Fn(&ServiceRequest) -> Result<String, ARError>>>;
type IdentifierStatic = Rc<Box<dyn Fn(&ServiceRequest) -> Result<String, ARError> + 'static>>;
    ...
    <T as Actor>::Context: ToEnvelope<T, ActorMessage>,
{
    ...
    pub fn new(store: Addr<T>) -> Self {
        let identifier = |req: &ServiceRequest| {
            ...
            let ip = connection_info.peer_addr().ok_or(ARError::IdentificationError)?;
            Ok(String::from(ip))
        };
        RateLimiter {
