        ...
        if let Some(mut con) = connection {
            match msg {
                ...
                ActorMessage::Update { key, value } => ActorResponse::Update(Box::pin(async move {
                    ...
                    let result = cmd.query_async::<MultiplexedConnection, usize>(&mut con).await;
                    match result {
                        Ok(c) => Ok(c),
                        Err(e) => Err(ARError::ReadWriteError(format!("{:?}", &e))),
                    }
                ...
                ActorMessage::Get(key) => ActorResponse::Get(Box::pin(async move {
                    let mut cmd = redis::Cmd::new();
                    cmd.arg("GET").arg(key);
                    let result = cmd.query_async::<MultiplexedConnection, Option<usize>>(&mut con).await;
                ...
                ActorMessage::Expire(key) => ActorResponse::Expire(Box::pin(async move {
                    let mut cmd = redis::Cmd::new();
                    cmd.arg("TTL").arg(key);
                    let result = cmd.query_async::<MultiplexedConnection, isize>(&mut con).await;
                    match result {
                        Ok(c) => {
                            if c > 0 {
                                ...
                                Err(ARError::ReadWriteError(
                                    "redis error: key does not exists or does not has a associated ttl.".to_string(),
                                ))
                            }
                        }
                        Err(e) => Err(ARError::ReadWriteError(format!("{:?}", &e))),
...
mod tests {
    ...
    async fn test_expiry() {
        ...
        let res3 = addr.send(ActorMessage::Expire("hello_test".to_string())).await;
