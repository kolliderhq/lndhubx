...
impl MemoryStore {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        debug!("Creating new MemoryStore");
        MemoryStore {
            inner: Arc::new(DashMap::<String, (usize, Duration)>::with_capacity(capacity)),
        }
    }
}
    ...
    fn handle(&mut self, msg: ActorMessage, ctx: &mut Self::Context) -> Self::Result {
        match msg {
            ...
            ActorMessage::Update { key, value } => match self.inner.get_mut(&key) {
                ...
                None => ActorResponse::Update(Box::pin(future::ready(Err(ARError::ReadWriteError(
            ...
            ActorMessage::Get(key) => {
                if self.inner.contains_key(&key) {
                    let val = match self.inner.get(&key) {
                        Some(c) => c,
                        None => {
                            return ActorResponse::Get(Box::pin(future::ready(Err(ARError::ReadWriteError(
                                "memory store: read failed!".to_string(),
                            )))))
                        }
                    };
            ...
            ActorMessage::Expire(key) => {
                let c = match self.inner.get(&key) {
                    Some(d) => d,
                    None => {
                        return ActorResponse::Expire(Box::pin(future::ready(Err(ARError::ReadWriteError(
                            "memory store: read failed!".to_string(),
                        )))))
                    }
                };
            ...
            ActorMessage::Remove(key) => {
                ...
                let val = match self.inner.remove::<String>(&key) {
                    Some(c) => c,
                    None => {
                        return ActorResponse::Remove(Box::pin(future::ready(Err(ARError::ReadWriteError(
