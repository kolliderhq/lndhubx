...
pub async fn store_quotes(dealer: &DealerEngine, client: &Client, bucket: &str) {
    ...
    let convertion_btc_usd = ConversionInfo::new(Currency::BTC, Currency::USD);
    let (btc_usd_rate, _btc_usd_fee) = dealer.get_rate(some_sats.clone(), convertion_btc_usd);

    let convertion_btc_eur = ConversionInfo::new(Currency::BTC, Currency::EUR);
    let (btc_eur_rate, _btc_eur_fee) = dealer.get_rate(some_sats, convertion_btc_eur);

    let one_usd = Money::new(Currency::USD, Some(dec!(1.0)));
    let convertion_usd_btc = ConversionInfo::new(Currency::USD, Currency::BTC);
    let (usd_btc_rate, _usd_btc_fee) = dealer.get_rate(one_usd, convertion_usd_btc);

    let one_eur = Money::new(Currency::EUR, Some(dec!(1.0)));
    let convertion_eur_btc = ConversionInfo::new(Currency::EUR, Currency::BTC);
    let (eur_btc_rate, _eur_btc_fee) = dealer.get_rate(one_eur, convertion_eur_btc);
