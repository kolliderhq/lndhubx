...
pub async fn pay_lnurl_withdrawal(
  ...
  match timeout(Duration::from_secs(5), response_rx.recv()).await {
    ...
    Ok(Some(Ok(Message::Api(Api::PaymentResponse(_))))) => return Ok(HttpResponse::Ok().json(json!({"status": "OK"}))),
    _ => {}
  };
  Err(ApiError::Comms(CommsError::ServerResponseTimeout))
}
...
pub async fn lnurl_pay_address(path: Path<String>, pool: WebDbPool) -> Result<HttpResponse, ApiError> {
  ...
  let callback = format!("https://lndhubx.kollider.xyz/api/pay/{:}", username);
  let max_sendable = 1000000000;
  let min_senable = 1000;
...
pub async fn pay_address(
  ...
  let response_filter: Box<dyn Send + Fn(&Message) -> bool> =
    Box::new(move |message| matches!(message, Message::Api(Api::InvoiceResponse(invoice)) if invoice.req_id == req_id));
